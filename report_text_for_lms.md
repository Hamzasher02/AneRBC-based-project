...
- Preprocessing Pipeline: Resized all raw images to 224x224 JPEG format in data/processed/.
...
- custom_cnn_3: Accuracy 59.33%, Precision (Macro) 0.5984, Recall (Macro) 0.5933, F1-Score (Macro) 0.5880, ROC-AUC 0.6315
- custom_cnn_4: Accuracy 66.67%, Precision (Macro) 0.6769, Recall (Macro) 0.6667, F1-Score (Macro) 0.6618, ROC-AUC 0.6967
- custom_cnn_5: Accuracy 62.67%, Precision (Macro) 0.6267, Recall (Macro) 0.6267, F1-Score (Macro) 0.6267, ROC-AUC 0.6873
...
- mobilenet_v2: Accuracy 75.33%, Precision (Macro) 0.7707, Recall (Macro) 0.7533, F1-Score (Macro) 0.7493, ROC-AUC 0.8228
- squeezenet1_0: Accuracy 74.00%, Precision (Macro) 0.7421, Recall (Macro) 0.7400, F1-Score (Macro) 0.7394, ROC-AUC 0.8455
- resnet18: Accuracy 70.00%, Precision (Macro) 0.7083, Recall (Macro) 0.7000, F1-Score (Macro) 0.6970, ROC-AUC 0.8084
...
Commands:
- Preprocess raw data: python scripts/download_data.py --data_root data/raw
- Train custom models: python scripts/train.py --model custom_cnn_4 --epochs 50 --batch_size 16 --workers 0 --patience 8
- Train pretrained models: python scripts/train.py --model mobilenet_v2 --pretrained --freeze --epochs 20 --batch_size 16 --workers 0 --patience 6
- Evaluate model on test set: python scripts/evaluate.py --model mobilenet_v2 --checkpoint checkpoints/mobilenet_v2_best.pth --workers 0
- Run XAI explanations: python scripts/explain.py --method gradcam --model mobilenet_v2 --checkpoint checkpoints/mobilenet_v2_best.pth --image data/processed/anaemic/165_a.jpg
